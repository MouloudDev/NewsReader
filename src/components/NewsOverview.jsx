...
export default function NewsOverview() {
  ...
  const newsCardStyles = {
    wrapperStyles: "flex flex-col justify-start p-1 rounded-lg w-full hover:bg-zinc-100 transition-all duration-300",
    imageStyles: "object-cover w-full h-64 rounded-lg xl:max-h-56",
      ...
      <div className="xl:col-span-4 grid content-between">
        ...
        <div className="grid content-between gap-2 md:max-xl:grid-rows-2 md:max-xl:grid-cols-2">
          {general.map((article) => {
            ...
            return (
              <div
                className="grid gap-2 grid-cols-1 p-1 border-b-2 border-b-zinc-200 rounded-md w-full xl:grid-cols-5 hover:bg-zinc-100 transition-all duration-300"
                key={id}
              >
                <div className="flex flex-col xl:col-span-3">
                  <div className="flex gap-1 items-center">
                    <img
                      src={sourceIcon}
                      alt="Source icon"
                      className="h-8 w-8 rounded-full border border-zinc-400 shadow-sm"
                    />
                    <span className="text-xl font-medium text-left">{author || source}</span>
                  </div>
                  <Link
                    to={`/article/${id}`}
